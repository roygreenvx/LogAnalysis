...
layui.use(['form','layer','jquery','cookie'],function(){
    ...
    $.cookie('signusername', null,{ expires: -1 , path: '/'});
    $.cookie('signpassword', null,{ expires: -1 , path: '/'});
    ...
    form.on("submit(login)",function(data){
        ...
        $.ajax({
            ...
            success: function (result) {
            	if(result){
            		$.cookie('signusername', data.field.userName, { expires: 7 , path: '/'});
            		$.cookie('signpassword', data.field.password, { expires: 7 , path: '/'});
